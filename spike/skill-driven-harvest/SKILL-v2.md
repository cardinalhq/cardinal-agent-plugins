# mechanize-compile (spike v2)

**Compile a Claude Code session into a Sentinel DAG candidate.** Spike-quality, for testing what a well-guided skill can produce from a real session before committing to phase 2 compiler design. Not a shipping skill.

v2 supersedes v1 by closing five gaps surfaced in EVAL.md and rationale.md meta-feedback: spill-to-disk collapsing, code-reading compression, tool-argument expression language, node-ID stability under iteration, and attachment chooser policy. Other guidance is preserved.
...
Also read: `spike/dag-harvest/FINDINGS.md` — empirical findings from the heuristic spike. Every rule in that document supersedes what a naive reading of the spec might suggest, EXCEPT F3, which is refined in "Spill-to-disk collapsing" below.

### Prior spike output — explicit policy (new in v2)

You MAY read `spike/dag-harvest/out/session-<label>-report.md` and related heuristic outputs from spike-1 as background context. They exist to give you the heuristic-computed edge lists, dead-end enumeration, and attachment metadata for the same session. Reading them is not compilation-by-transcription: your job is still to work from the source JSONL. Use the heuristic report to verify your tool-call ordinals, cross-check dead-end detection, and confirm attachment IDs. Do NOT copy prose from it into rationale.md.

You MAY NOT read `spike/skill-driven-harvest/out/sentinel.yaml` or `spike/skill-driven-harvest/out/rationale.md` before compiling. Those are v1's outputs and would bias structural decisions. If you're recompiling as part of a spike-2 eval, treat them as forbidden until your own YAML is written and saved.
...
### Stage 1.5 — Recognize spill-to-disk pairs (new in v2, refines F3)
...
**F3 REFINEMENT.** Spike-1's F3 rule "filter tool calls whose input references `~/.claude/`" is too broad. Apply this refined rule:
...
- **COLLAPSED** (new in v2) — spill-projection Bash call absorbed into a preceding REQUIRED/SUPPORTING node per Stage 1.5. Do NOT double-classify.
...
### Stage 2.5 — Code-reading compression policy (new in v2)
...
v1's default was to compress this into structured operator inputs (e.g., `dimensionalBreakdown`, `withinSeriesFlatnessTolerance` — the human's *conclusion* from code reading becomes an input). That works but has fidelity cost: a future operator running the Sentinel on a different metric must repeat the code reading themselves and pick the same inputs.
...
- Add an `llm` node whose task is "read the emission-site code under `${inputs.codeRepoPath}` for `${inputs.metricName}` and return a structured claim about which dimensions should vary and what tolerance means 'flat'."
- Evidence: the grep-result summary. Model policy: analytical-small.
- **Pros:** preserves the operator's judgment mechanically; a future run against a different metric re-does the analysis.
- **Cons:** LLM node in every execution; must justify per §32 (deterministic alternative considered, reason rejected); output must be schema-valid and support `inconclusive`.
- **Pick this when:** the code-reading judgment is generalizable across metrics/services and the downstream degeneracy/comparison logic depends on it.
...
- **Pick this when:** the operator was extracting NUMBERS or ENUMS from code (e.g., `LevelMaxAgeCap = 1h`), not making qualitative judgments.

**Option C — compress into structured operator inputs (v1 default).**
- Encode the operator's *conclusion* as a plain input; drop the code-reading tool calls; add an OPTIONAL `locate-metric-emission-context` grep node for reviewer context only.
- **Pros:** simplest Sentinel; no LLM cost; a specialist reviewer sees the reasoning in the input names.
- **Cons:** genuine fidelity loss. A generalist running this Sentinel against an unfamiliar metric must reproduce the code reading themselves and pick the same input values without guidance.
...
### Stage 4.5 — Attachment chooser (new in v2)
...
→ Emit the attachment as a Sentinel input of type `image`, `pdf`, or `binary`. Downstream nodes reference it via `${inputs.<name>}`. Note: v0 has no multimodal function-node runtime (§31), so a function node cannot consume the attachment; only an `llm` node (§12) may reference it, and even then only when a multimodal capability is declared as required.
...
Do not generate prose that purports to summarize what a screenshot shows and use that prose as a fact in the Sentinel. This is a hallucination cloaked as compilation (§29 stage 3, final paragraph).
...
7. **Attachment discipline** — did any attachment content get inlined as evidence text? If yes, remove it. Was the Stage 4.5 chooser applied? If not, apply it now.
8. **Spill-projection collapse discipline** — is any spill-projection Bash call still present as its own tool node? If yes, collapse into its preceding node.

### Stage 6 — Iterate (max 3 rounds) with node-ID stability freeze (revised in v2)
...
## Expression language (new in v2, resolves v1 §13 ambiguity)
...
**B. Tool-argument and finding-body expressions** (`${...}` interpolations in `arguments:`, `title:`, `dedupeKey:`, `attributes:`, `severityExpression:`) — §13 does NOT govern these. The pragmatic v2 ruling, pending spec clarification:
...
- Do NOT read `spike/skill-driven-harvest/out/sentinel.yaml` or its rationale before writing your own.
...
- A `rationale.md` that a human reader could use to audit every compilation decision, including the code-reading option chosen (Stage 2.5) and the attachment disposition (Stage 4.5).
