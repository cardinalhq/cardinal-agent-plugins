...
Phase → Cardinal event mapping (spec docs/specs/omnigent-adapter.md):
  ...
  request      → cardinal.git_state       initiative facts from the labels
                                           ...
                                           cardinal.branch); user-turn boundary
  llm_response → api_request +
                 cardinal.turn_usage      PER-TURN granularity (documented
                                           divergence from the CLI adapters'
                                           per-model-call events); cost_usd
                                           from the delta of the session's
                                           cumulative context.usage
                                           .total_cost_usd (tracked via
                                           omnigent state_updates with an
                                           in-process mirror), falling back
                                           to core pricing tables on
                                           data.model
...
cardinal.subagent_usage (spec row "response of sub-agent sessions") is
deliberately NOT emitted yet: which omnigent event marks a child-agent
boundary is engine-dependent (spec open question 1) and needs a
per-engine mapping pass before we can emit an honest stream.
...
errors and returns None (abstain → ALLOW). The only non-None return is
the llm_response ALLOW carrying `state_updates` for the cost-delta
anchor — state_updates are applied on ALLOW, never on abstain.
...
_PLUGIN_VERSION_FALLBACK = "0.1.0"
...
# In-process per-session mirror: sequence counters + cost anchor. Policies
# run in the long-lived omnigent server process, so module state is a
# valid cache; omnigent session_state (via state_updates) is the durable
# copy where the engine supports it (scoping is engine-dependent — spec
# open question 2).
...
# ---------------------------------------------------------------------------
# request → cardinal.git_state (labels convention) + user-turn boundary
# ---------------------------------------------------------------------------
...
def _handle_request(event: Any, config: dict[str, Any], session_id: str) -> None:
    ...
    _emit(event, config, [otlp.log_record("cardinal.git_state", attrs, time.time_ns())])
...
def _normalize_usage(raw: dict[str, Any]) -> dict[str, Any]:
    """Map omnigent's llm_response usage payload (per-TURN totals) onto the
    Cardinal contract's canonical bucket names, probing the cache-bucket
...
def _fallback_cost(model: str | None, usage: dict[str, Any]) -> float | None:
    """Core pricing tables keyed on data.model — used when the session's
    cumulative total_cost_usd delta is unavailable. Unpriced models
    (e.g. Anthropic SKUs, which have no core table) return None and the
    attribute is skipped — no misleading zero rows."""
    for table in pricing.PROVIDER_TABLES.values():
        cost = pricing.compute_cost_usd(model, usage, table)
        if cost is not None:
            return cost
    return None
...
def _handle_llm_response(
    ...
    records = [
        ...
        otlp.log_record("cardinal.turn_usage", {
            **base,
            ...
            # Per-turn granularity marker — the CLI adapters emit
            # per-model-call where available; omnigent's llm_response is
            # cumulative across the turn's API calls (documented divergence).
    ...
    # ALLOW (not abstain) so the engine applies the cost anchor — omnigent
    # withholds state_updates on ASK and cannot apply them on None.
    return {
        "result": "ALLOW",
        "state_updates": [
            {"action": "set", "key": COST_ANCHOR_KEY, "value": current_total},
        ],
    }
...
def _handle_tool_call(event: Any, config: dict[str, Any], session_id: str) -> None:
    ...
    if mcp is not None:
        ...
        classified = bashclass.classify_bash_command(_shell_command(_events.data(event)))
...
def telemetry_policy(event: Any, config: Any = None) -> dict[str, Any] | None:
    ...
    opinion). Never blocks, never asks."""
    try:
        ...
        if phase == "request":
            ...
            return _handle_llm_response(event, cfg, session_id)
