...
- `cardinal.decision` per structured_output entry when the session is
  terminal, re-sent only if that decision's attributes changed (a session
  that resumes and finishes again).

State is updated only after ingest accepts the records, so a failed POST
is retried on the next cycle. Untracked sessions last updated before the
lookback window are ignored, and tracked terminal sessions are pruned
after it (plus a day), which keeps re-listing from re-emitting them.
"""
...
from .sessions import Session, normalize
...
class Poller:
    ...
    def poll_once(self) -> PollResult:
        ...
        updated_after = None
        if api == "v3":
            updated_after = max(self.state.hwm(api) or floor, floor)
    ...
    def _refresh_unlisted(self, seen: set, now: float, result: PollResult) -> None:
        ...
        for sid, entry in list(self.state.sessions.items()):
            if sid in seen or entry.get("terminal") or entry.get("stale"):
                continue
            ...
            try:
                ...
                continue
            if raw is None:
                log.info("session %s: no longer returned by the API", sid)
                entry["terminal"] = True
                entry["gone"] = True
                continue
    ...
    def _prune(self, now: float) -> None:
        horizon_ns = int((now - (self.options.lookback_days + 1) * DAY) * NS)
        for sid in list(self.state.sessions):
            entry = self.state.sessions[sid]
            updated = entry.get("updated_ns")
            done = entry.get("terminal") or entry.get("stale")
    ...
    def _handle(self, session: Session, *, from_list: bool, result: PollResult) -> None:
        ...
        if (
            ...
            return  # finished and unchanged since we finalized it
        ...
        def facts_for(url: str) -> Dict[str, Any]:
        ...
        skipped = 0
        if finalize and self.options.emit_decisions:
            built, reasons, problem = telemetry.build_decisions(
                session.structured_output if session.has_structured_output else None
            )
            if problem:
            ...
            skipped = len(reasons)
            primary = facts_for(session.pr_urls[0]) if session.pr_urls else {}
            for decision in built:
                attrs = telemetry.decision_attrs(sid, decision, primary)
                digest = telemetry.fingerprint(attrs)
