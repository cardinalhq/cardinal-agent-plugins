...
class ExecutionGraphConnectionTests(unittest.TestCase):
    """_otel_settings.execution_graph_connection (Phase 1 execution-graph
    endpoint discovery — docs/local-notes/plans/agent-execution-graph.md).
    Unit-level, against settings_env dicts directly: no cardinal_core.envelope
    import is involved here (only cardinal_core.otlp.IngestConnection), so
    this doesn't need the Python 3.11+ floor the emitter itself requires."""

    def setUp(self):
        self.tmp = TemporaryDirectory()
        self.home = Path(self.tmp.name)
        self.otel = _load_otel_settings(self.home)

    def tearDown(self):
        self.tmp.cleanup()

    def test_no_endpoint_configured_is_none(self):
        self.assertIsNone(self.otel.execution_graph_connection({}))

    def test_falls_back_to_otlp_endpoint_base_stripping_v1_logs(self):
        conn = self.otel.execution_graph_connection({
            "OTEL_EXPORTER_OTLP_ENDPOINT": "http://collector:4318/v1/logs",
        })
        self.assertIsNotNone(conn)
        self.assertEqual(conn.endpoint, "http://collector:4318")

    def test_otlp_endpoint_with_no_v1_suffix_used_verbatim(self):
        conn = self.otel.execution_graph_connection({
            "OTEL_EXPORTER_OTLP_ENDPOINT": "http://collector:4318",
        })
        self.assertEqual(conn.endpoint, "http://collector:4318")

    def test_otlp_endpoint_bare_v1_suffix_stripped(self):
        conn = self.otel.execution_graph_connection({
            "OTEL_EXPORTER_OTLP_ENDPOINT": "http://collector:4318/v1",
        })
        self.assertEqual(conn.endpoint, "http://collector:4318")

    def test_explicit_override_wins_over_otlp_endpoint(self):
        conn = self.otel.execution_graph_connection({
            "OTEL_EXPORTER_OTLP_ENDPOINT": "http://collector:4318/v1/logs",
            "CARDINAL_EXECUTION_GRAPH_ENDPOINT": "http://execution-graph:9999",
        })
        self.assertEqual(conn.endpoint, "http://execution-graph:9999")

    def test_explicit_override_alone_is_sufficient(self):
        # No OTEL_EXPORTER_OTLP_ENDPOINT at all -- the override alone must
        # still produce a connection (rule 1 is independent of rule 2).
        conn = self.otel.execution_graph_connection({
            "CARDINAL_EXECUTION_GRAPH_ENDPOINT": "http://execution-graph:9999/",
        })
        self.assertIsNotNone(conn)
    ...
    def test_auth_header_and_key_match_ingest_connection(self):
        settings_env = {
            "OTEL_EXPORTER_OTLP_ENDPOINT": "http://collector:4318/v1/logs",
            "OTEL_EXPORTER_OTLP_HEADERS": "x-cardinalhq-api-key=sekrit,x-other=v1",
        }
        conn = self.otel.execution_graph_connection(settings_env)
        self.assertEqual(conn.api_header, "x-cardinalhq-api-key")
        self.assertEqual(conn.api_key, "sekrit")
        self.assertEqual(dict(conn.extra_headers), {"x-other": "v1"})
