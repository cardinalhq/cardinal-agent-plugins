...
The hook is the only thing that writes or emits a decision. In Run Everything mode (unsandboxed) or on Windows, the CLI still has no side effects, so nothing is sent twice. The hook only accepts a marker when the shell command actually runs `cardinal-decision record` (optionally as `python3 "<path>/cardinal-decision" record …`), and it accepts one marker per invocation. Markers that are echoed, printed, grepped, or cat'ed from saved output are ignored. If recording fails unexpectedly, the agent is told the decision was **not** recorded and why. If you aren't connected, decisions are only saved to the local ledger.

The hook runs outside the agent sandbox (the existing hooks already write under `~/.cursor/cardinal/` and POST to ingest from there). The `git` calls and the `gh`-based PR lookup therefore **still run unsandboxed**, with your `gh` credentials. To stay fast, the recorder never waits on `gh`. It uses the PR already cached by a prompt's `git_state` lookup and, if that cache entry is missing or stale, starts a detached background refresh. So the first decision on a new branch may be sent without a PR. The code-cluster tree listing is capped at 1.5s and the decision POST at 1.5s, which keeps recording within a 5s hook timeout.

**After upgrading, re-run `python3 scripts/cardinal-connect --rotate`** (add `--project` if you use it). Installs from before decision capture registered `postToolUse` with a 5s timeout; re-running writes the current 15s timeout. Recording is designed to fit in 5s, but the extra headroom covers slow ingest or a slow `git`.
